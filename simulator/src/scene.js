...
(function () {
  ...
  function buildCell() {
    ...
    const cb = CELL.controlBox, cam = CELL.camera;
...
(ID ${tray.length} x ${tray.width} x ${tray.depth} mm)`],
  ...
  const LAYER_HOLD_S = 1.0;   // simulation seconds a finished layer stays on show before the next layer's slots appear
  function updatePlan() {
    if (!state.sim || !planSlots.length) return;
    const pat = state.pattern, perLayer = pat.perLayer * pat.trays, sim = state.sim;
    let count = sim.crate.changing ? pat.total : sim.crate.count;
    // A layer just finished (and the tray is not full): hold it on show for a moment
    // so the products can be seen to have gone in (Chris, 2026-09-22).
    if (count !== state.planCount) {
      if (count > 0 && count < pat.total && count % perLayer === 0) state.layerHold = { count, until: sim.time + LAYER_HOLD_S };
      state.planCount = count;
    }
    const holding = state.layerHold && sim.time < state.layerHold.until && count >= state.layerHold.count;
    if (holding) count = state.layerHold.count;
    const layer = holding ? count / perLayer - 1 : Math.min(pat.layers - 1, Math.floor(count / perLayer));
    ...
    $("planNote").textContent = sim.crate.changing ? "Tray full: changing trays"
      : holding ? `Layer ${layer + 1} of ${pat.layers} packed` : `Packing layer ${layer + 1} of ${pat.layers}`;
  }
...
(Product looked at: ${state.sku.name}${state.sku.tray.name ? ", " + state.sku.tray.name : ""})` : "");
  ...
  async function send() {
    ...
    try {
      const response = await fetch(contact.endpoint, {
        method: "POST", mode: contact.noCors ? "no-cors" : "cors",
        headers: { "Content-Type": contact.noCors ? "text/plain" : "application/json", Accept: "application/json" },
  ...
  $("contactSend").addEventListener("click", send);
